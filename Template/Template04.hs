...
numStep (_,sk,_,hp,_,_) _ = error ("the number of element in the stack is not one"
                                 ++ "\n"
                                 ++ (iDisplay $ showStack hp sk))

apStep :: TiState -> Addr -> Addr -> TiState
apStep (op,(a:sk),dp,hp,gb,sic) a1 a2
  = if isIndNode node
       then (op, (a:sk),dp,hp',gb,sic)
       else (op, (a1:a:sk),dp,hp,gb,sic)
  where
    node = hLookup a2 hp
...
scStep (op, sk,dp,hp,gb,sic) sc_name arg_names body
...
primIf (op,[a,a1,a2,a3],dp,hp,gb,sic)
  | isDataNode s = (op,[a3],dp,new_hp,gb,sic)
  | otherwise = (op,[s_addr],[a1,a2,a3]:dp,hp,gb,sic)
  where
...
casePairStep (op,[a,a1,a2],dp,hp,gb,sic)
  | not $ isDataNode p = (op,[p_addr],[a2]:dp,hp,gb,sic)
  | otherwise = (op,[new_f_addr],dp,new_hp,gb,sic)
  where
...
caseListStep (op,[a,a1,a2,a3],dp,hp,gb,sic)
  ...
  | otherwise = (op,[a3],dp,new_hp,gb,sic)   --"new_hp" depends on the kind of "pack tag"
  where
